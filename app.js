...
/* render */
function renderFolders(){F.innerHTML=visible().map(b=>'<li><button type="button" aria-expanded="false" aria-controls="p-'+b.id+'" data-w="'+b.id+'">'+ICON+'<span class="lbl">'+esc(b.name)+'</span><span class="n">'+b.items.length+'</span></button></li>').join('');}
function rowHTML(it,i,prev){if(Array.isArray(it)){const first=!prev||!Array.isArray(prev)||prev[0]!==it[0];return '<li'+(first?' class="first"':'')+'>'+(first?'<span class="d">'+esc(it[0])+'</span>':'')+'<span>'+(it[2]?'<a href="'+it[2]+'" data-u="'+it[2]+'">'+esc(it[1])+'</a>':esc(it[1]))+'</span></li>';}
 return '<li class="first role"><span class="d">'+esc(it.y)+'</span><span><a href="#" class="lbl" data-role="'+i+'">'+esc(it.label)+'</a></span></li>';}
...
/* folders */
F.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;const id=b.dataset.w;
...
function postHTML(u,lab,noMedia){const m=u.match(/status\/(\d+)/);const p=m&&PREVIEWS[m[1]];
 ...
 const body=lab?p.text.split(/\n/)[0]:p.text;const med=noMedia?[]:(p.media||[]);
 const more=(lab||!p.thread)?'':p.thread.map(t=>'<div class="cont">'+mediaHTML(t.media||[],u)+'<p class="txt">'+esc(t.text)+'</p></div>').join('');
 return '<div class="post"><p class="who"><span>X · @emilylai</span><span>'+fmt(p.date)+'</span></p>'+mediaHTML(med,u)+'<p class="txt">'+esc(body)+(lab?' <a href="'+u+'" target="_blank" rel="noopener">The rest is on X ↗</a>':'')+'</p>'+more+(lab?'':'<p class="stats">'+p.likes.toLocaleString()+' likes</p><p class="out"><a href="'+u+'" target="_blank" rel="noopener">Open on X ↗</a></p>')+'</div>';}
...
P.addEventListener('click',e=>{
 ...
 const r=e.target.closest('a[data-role]');if(r){e.preventDefault();const pane=r.closest('.pane'),b=BUCKETS.find(x=>'p-'+x.id===pane.id),it=b.items[+r.dataset.role];const rd=select(pane,r.closest('li'));
  rd.innerHTML='<p class="who"><span>'+esc(it.y)+'</span></p><p class="rl">'+esc(it.label)+'</p>'+(it.desc&&it.desc.length?'<ul class="bul">'+it.desc.map(d=>'<li>'+md(d)+'</li>').join('')+'</ul>':'')+logosHTML(it.label)+(it.link?'<p class="out"><a href="'+it.link+'" target="_blank" rel="noopener">Open '+esc(new URL(it.link).host)+' ↗</a></p>':'')+(it.series?'<div class="row" role="list">'+it.series.map(cardHTML).join('')+'</div><div class="stagebox"></div>':'')+(it.posts||[]).map(u=>postHTML(u,!!it.safe,!!it.safe)).join('');mobileBack(pane,rd);return;}
 const a=e.target.closest('a[data-u]');if(!a)return;e.preventDefault();const pane=a.closest('.pane'),u=a.dataset.u,lab=false;const rd=select(pane,a.closest('li'));
...
(function(){const pal=document.getElementById('palette'),form=pal.querySelector('.search'),q=document.getElementById('q'),hits=form.querySelector('.hits'),btn=document.querySelector('.sbtn');pal.addEventListener('click',e=>{if(e.target===pal)hide();});pal.querySelector('.pclose').addEventListener('click',()=>hide());
 ...
 function apply(v){const s=v.trim().toLowerCase();let total=0;BUCKETS.forEach(b=>{const pane=document.getElementById('p-'+b.id);if(!pane)return;let n=0;const lis=pane.querySelectorAll('.list li');lis.forEach((li,i)=>{const ok=!s||textOf(b.items[i]).toLowerCase().includes(s);li.classList.toggle('hide',!ok);if(ok)n++;});total+=n;
...
document.querySelectorAll('a.mail').forEach(a=>{const u=[...a.dataset.u].reverse().join(''),d=[...a.dataset.d].reverse().join('');const addr=u+'@'+d;a.href='mailto:'+addr;a.textContent=addr;});
})();
