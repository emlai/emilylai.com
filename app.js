...
function postHTML(u,lab,noMedia,cut,noThread){const m=u.match(/status\/(\d+)/);const p=m&&PREVIEWS[m[1]];
...
function streamHTML(it){if(streams.has(it))return streamStart(streams.get(it));const items=[];for(const u of it.posts||[]){const id=(u.match(/status\/(\d+)/)||[])[1];const p=id&&PREVIEWS[id];const so=!!(it.solo&&it.solo[id]);items.push({d:p?p.date:'0000',html:postHTML(u,!!it.safe,!!it.safe||so,it.cuts&&it.cuts[id],so)});}
