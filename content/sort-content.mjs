...
const key=r=>{const ids=[...String(r[5]).matchAll(/status\/(\d+)/g)].map(m=>m[1]);const ds=ids.map(i=>(PV[i]||{}).date).filter(Boolean).sort();if(ds.length)return ds[ds.length-1];const y=String(r[2]).match(/\d{4}/g)||['0000'];return y[y.length-1]+'-12-31';};
