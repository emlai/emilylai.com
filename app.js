...
function updateDividers(){if(isMobile())return;P.querySelectorAll('.pane.on .divider').forEach(d=>{const width=d.parentElement.clientWidth;if(!width)return;d.setAttribute('aria-valuemin',Math.round(210/width*100));d.setAttribute('aria-valuemax',Math.round((width-268)/width*100));d.setAttribute('aria-valuenow',Math.round(d.previousElementSibling.clientWidth/width*100));});}
...
WIN.inert=true;
function visit(id){const was=unlocked();seen.add(id);store.set('seen',[...seen]);if(!was&&unlocked())renderFolders();if(BUCKETS.find(b=>b.id===id)?.egg)store.set('eggOpened',true);}
function closeWindow(immediate=false){if(immediate!==true&&isMobile()&&open.length){Motion.folderClose(()=>closeWindow(true));return;}const id=active;open=[];active=null;sync(false);F.querySelector('[data-w="'+id+'"]')?.focus({preventScroll:true});}
...
F.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;const id=b.dataset.w;const was=unlocked();seen.add(id);store.set('seen',[...seen]);if(!was&&unlocked())renderFolders();if((BUCKETS.find(x=>x.id===id)||{}).egg)store.set('eggOpened',true);
 if(isMobile()){const origin=b.getBoundingClientRect();open=[id];active=id;document.querySelectorAll('.pane').forEach(p=>p.classList.remove('has-cur'));sync(true);Motion.folderOpen(origin);return;}
...
function streamHTML(it){if(streams.has(it))return streamStart(streams.get(it));const items=[];for(const u of it.posts||[]){const id=(u.match(/status\/(\d+)/)||[])[1];const p=id&&PREVIEWS[id];items.push({d:p?p.date:'0000',html:postHTML(u,!!it.safe,!!it.safe,it.cuts&&it.cuts[id])});}
 ...
 if(it.embeds?.length)items.sort((a,b)=>b.d.localeCompare(a.d));const html=items.map(x=>x.html);streams.set(it,html);return streamStart(html);}
...
function observeStream(rd,it){const marker=rd.querySelector('.more-posts'),items=it&&streams.get(it);if(!marker||!items)return;let next=0;
 const observer=new IntersectionObserver(entries=>{if(!entries.some(e=>e.isIntersecting)||!rd.closest('.pane.on'))return;marker.insertAdjacentHTML('beforebegin',items.slice(next,next+2).join(''));next+=2;if(next>=items.length){observer.disconnect();marker.remove();streamObservers.delete(rd);}else{observer.unobserve(marker);observer.observe(marker);}},{root:rd,rootMargin:'600px 0px'});
...
P.addEventListener('click',e=>{
 const bk=e.target.closest('.back');if(bk){backToList(bk.closest('.pane'));return;}
 ...
 const r=e.target.closest('a[data-role]');if(r){e.preventDefault();const pane=r.closest('.pane'),b=BUCKETS.find(x=>'p-'+x.id===pane.id),it=b.items[+r.dataset.role];const rd=select(pane,r.closest('li'));
  rd.innerHTML='<p class="who"><span>'+esc(it.y)+'</span></p><p class="rl">'+esc(it.label)+'</p>'+descHTML(it.desc)+logosHTML(it.label)+seriesHTML(it)+imagesHTML(it)+(it.link?'<p class="out"><a href="'+it.link+'" target="_blank" rel="noopener">Open '+esc(new URL(it.link).host)+' ↗</a></p>':'')+streamHTML(it);mobileBack(pane,rd,it);return;}
...
(function(){let drag=null,sx,sw;const v=store.get('listw',null);if(v)WIN.style.setProperty('--list',v);
 ...
 P.addEventListener('pointermove',e=>{if(!drag)return;const pw=drag.closest('.pane').getBoundingClientRect().width;let w=sw+(e.clientX-sx);w=Math.max(210,Math.min(w,pw-268));WIN.style.setProperty('--list',(w/pw*100).toFixed(1)+'%');updateDividers();});
