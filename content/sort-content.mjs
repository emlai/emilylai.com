...
const chronological=new Set(['growth experiments','lab experiments','learning']);
const newest=r=>{const times=String(r[5]).split(/\n/).map(u=>socialTimestamp(u)).filter(Boolean);return times.length?Math.max(...times):Date.parse((String(r[2]).match(/\d{4}/g)||['0000']).at(-1)+'-12-31')||0;};
const buckets=[...new Set(R.map(r=>r[0]))];let fin=[];
for(const b of buckets){let rs=R.filter(r=>r[0]===b);if(chronological.has(b))rs.sort((a,c)=>newest(c)-newest(a));else if(b!=='work'&&!rs.some(r=>/curated order/i.test(r[9]||'')))rs.sort((a,c)=>key(c).localeCompare(key(a)));else rs.sort((a,c)=>+a[1]-+c[1]);rs.forEach((r,i)=>r[1]=String(i+1));fin=fin.concat(rs);}
