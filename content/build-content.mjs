...
const ids={'work':'work','growth experiments':'growth','lab experiments':'lab','learning':'learn','speaking':'talks','side projects':'side'};
const buckets={};
...
const order=['work','growth experiments','lab experiments','learning','speaking','side projects'];
const out=order.filter(n=>buckets[n]).map(n=>({id:buckets[n].id,name:n,items:buckets[n].items.sort((a,b)=>a.o-b.o).map(x=>x.v)}));
