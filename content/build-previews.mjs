...
for(const id of ids){const t=byId.get(id);if(!t){if(FX[id])prev[id]=FX[id];else missing.push(id);continue;}
 ...
 if(th.length)p.thread=th.sort((a,b)=>a.id_str.localeCompare(b.id_str)).map(k=>{const o={text:clean(k.full_text)};const mm=mediaOf(k,k.id_str);if(mm.length)o.media=mm;return o;});
