...
function revealDiscovery(){if(!pendingDiscovery)return;pendingDiscovery=false;const egg=F.querySelector('.egg');egg?.classList.add('just-unlocked');egg?.addEventListener('animationend',()=>egg.classList.remove('just-unlocked'),{once:true});}
...
function postHTML(u,lab,noMedia,cut){const m=u.match(/status\/(\d+)/);const p=m&&PREVIEWS[m[1]];
 ...
 const more=(lab||cutted||!p.thread)?'':p.thread.map(t=>'<div class="cont">'+mediaHTML(t.media||[],u)+'<p class="txt">'+linkedText(t.text)+'</p></div>').join('');
 return '<div class="post"><p class="who"><span>X · @'+esc(p.author||'emilylai')+'</span><span>'+fmt(p.date)+'</span></p>'+mediaHTML(med,u)+'<p class="txt">'+linkedText(body)+(lab?' <a href="'+u+'" target="_blank" rel="noopener">The rest is on X ↗</a>':'')+'</p>'+more+(lab?'':'<p class="stats">'+p.likes.toLocaleString()+' likes</p><p class="out"><a href="'+u+'" target="_blank" rel="noopener">'+(cutted?'Read the rest on X ↗':'Open on X ↗')+'</a></p>')+'</div>';}
...
function imagesHTML(it){return (it.images||[]).filter(src=>src!==it.cover?.src).map(src=>'<div class="media">'+imageHTML(src)+'</div>').join('');}
...
function streamHTML(it){if(streams.has(it))return streamStart(streams.get(it));const items=[];for(const u of it.posts||[]){const id=(u.match(/status\/(\d+)/)||[])[1];const p=id&&PREVIEWS[id];items.push({d:p?p.date:'0000',html:postHTML(u,!!it.safe,!!it.safe,it.cuts&&it.cuts[id])});}
 ...
 if(it.embeds?.length)items.sort((a,b)=>b.d.localeCompare(a.d));let year='';const html=items.map(x=>{const y=x.d.slice(0,4),heading=it.yearSections&&y!==year?'<div class="year-break"><h3>'+esc(y)+'</h3><p>'+(year?'Five years earlier · a separate fast':'A new fast')+'</p></div>':'';year=y;return heading+x.html;});streams.set(it,html);return streamStart(html);}
...
P.addEventListener('click',e=>{
 ...
 const r=e.target.closest('a[data-role]');if(r){e.preventDefault();const pane=r.closest('.pane'),b=BUCKETS.find(x=>'p-'+x.id===pane.id),it=b.items[+r.dataset.role];const rd=select(pane,r.closest('li'));
  rd.innerHTML=coverHTML(it)+'<p class="who"><span>'+esc(it.y)+'</span></p><p class="rl">'+esc(it.label)+'</p>'+descHTML(it.desc)+logosHTML(it.label)+seriesHTML(it)+imagesHTML(it)+deckHTML(it)+(it.link?'<p class="out"><a href="'+it.link+'" target="_blank" rel="noopener">Open '+esc(new URL(it.link).host)+' ↗</a></p>':'')+streamHTML(it);mobileBack(pane,rd,it);return;}
