...
for(const b of buckets){let rs=R.filter(r=>r[0]===b);if(b!=='work'&&!rs.some(r=>/curated order/i.test(r[9]||'')))rs.sort((a,c)=>key(c).localeCompare(key(a)));else rs.sort((a,c)=>+a[1]-+c[1]);rs.forEach((r,i)=>r[1]=String(i+1));fin=fin.concat(rs);}
